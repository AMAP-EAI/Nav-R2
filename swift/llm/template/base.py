...
class Template(ProcessorMixin):
    ...
    def process_images_specific_on_PAI(self, images):
        ...
        for i in range(len(images)):
            ret[i]['path'] = images[i]['path'].replace(
                "/data/oss_bucket_0/gaoli/vlnce_data/objectnav_ovon", "/mnt/nas-data-3/xiangwentao.xwt/data/gaoli/vlnce_data/objectnav_ovon/objectnav_ovon"
            )
        return ret
    ...
    ) -> None: # train_datasets的下标每次变化一次都会执行一次这个函数去预处理数据，只有从train_datasets中取数据时才会临时执行一次这个函数
        if self.is_on_PAI:
            # print("================")
            # print('on PAI right now \n')
            # print()
            # """
            #     “/data/oss_bucket_0/gaoli/vlnce_data/objectnav_ovon” -> “/mnt/nas-data-3/xiangwentao.xwt/data/gaoli/vlnce_data/objectnav_ovon/objectnav_ovon”
            # """
